...
select DOEID,TEMPNITE,HEATHOME
from dir.r2015;
...
select r.TEMPNITE,r.HEATHOME,wt.NWEIGHT,wt.weight_value,wt.WEIGHT
from recs1 r
right join wt
on wt.DOEID = r.DOEID
...
outfile='/folders/myfolders/STATS506/PS6/PS6_q3a.csv' replace;
run;
...
create table recs as
select DOEID,DIVISION,TEMPNITE,TEMPHOME,TEMPGONE from dir.r2015;
...
select r.DIVISION,r.TEMPNITE,r.TEMPHOME,r.TEMPGONE,
       wt.NWEIGHT,wt.weight_value,wt.WEIGHT
from recs r
right join wt 
on r.DOEID = wt.DOEID
...
outfile='/folders/myfolders/STATS506/PS6/PS6_q3b.csv' replace;
run;
